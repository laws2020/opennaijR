---
title: "Fetching with nga_cbn()"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Fetching with nga_cbn()}
...
`nga_cbn()` is the primary data retrieval function in opennaijR. It connects your R
session directly to the Central Bank of Nigeria (CBN) data infrastructure, handles
all cleaning and formatting automatically, and delivers an analysis-ready object --
all in one call.

The function name follows the opennaijR convention for all African data sources:
...
When opennaijR expands to other countries, the same pattern applies consistently:
...
A user who learns `nga_cbn()` already understands every future function.
...
|----------|------|---------|--------------|
| `dataset` | `character` | required | Dataset key or alias from `discover_datasets()` |
...
| `filters` | `list` | `list()` | Advanced SDMX filter criteria |
...
`nga_cbn()` works in two distinct modes. Understanding the difference is the
most important thing to learn about this function.
...
Instead, `nga_cbn()` silently created a variable called `inflation` in your
global environment and placed the data there. The function returned the
string `"inflation"` invisibly -- just the name, not the data.
...
| Inside `lapply()` or a loop that returns results | `auto.assign = FALSE` |
| Building a pipeline with `|>` | `auto.assign = FALSE` |

> **Important:** Never use `auto.assign = TRUE` inside a function body. It writes
> to `.GlobalEnv` silently, which produces hard-to-trace side effects. Always use
> `auto.assign = FALSE` in non-interactive code.

---

## Part 2 -- Loading Multiple Datasets

This is where `auto.assign = TRUE` becomes genuinely powerful. You can retrieve
several datasets in a loop without writing a separate assignment for each one.
...
Each dataset key becomes its own variable name automatically. A researcher
building a macro model can load their full data universe in three lines.

You can also use `lapply()` for the same result:

```{r lapply-load}
invisible(lapply(datasets, nga_cbn))
```

`invisible()` suppresses the character vector of names that `lapply()` would
...
Use `from` and `to` to retrieve only the period you need. Both arguments accept
either a character string in `"YYYY-MM-DD"` format or an R `Date` object.
...
By default, `nga_cbn()` returns every indicator in a dataset. Use `variables`
to retrieve only the columns you need. This is useful when a dataset is wide
and you only care about one or two series.
...
opennaijR caches every download locally using the `pins` package. After the
first call, subsequent calls to the same dataset load from disk and are nearly
instant -- no network request is made.
...
The CBN periodically revises historical data and publishes new monthly figures.
Your local cache does not update automatically. Use `refresh = TRUE` when:
...
This is the output that `apply_projection()` and `derive_measure()` expect.
...
`raw = TRUE` returns the API response exactly as received -- messy column names,
string dates, and all. Use this when:
...
By default, `auto.assign = TRUE` writes to `.GlobalEnv`. You can redirect
this to a different environment using `env`.
...
This keeps your global environment clean when working with many datasets,
and is the correct pattern for package development or Shiny applications
where polluting `.GlobalEnv` is unacceptable.

---

## Part 8 -- Full Pipeline Integration

`nga_cbn()` with `auto.assign = FALSE` slots directly into a pipeline:
...
will follow the same interface as `nga_cbn()`. The arguments, the
caching behaviour, the `auto.assign` pattern, and the canonical output
will all work identically. Only the function name changes.

```{r future-pattern, eval=FALSE}
# Ghana -- Bank of Ghana
